...
async function renderLichTuan(lop, hvList, selectedNgay, activeCaId){
  ...
  const gvChips = [];
  const btnThemGV = USER.role==='admin' ? `<button class="btn-add-pro" onclick="openModalLop('${lop.lopId}')" title="Thêm/sửa giáo viên chính, giáo viên bản xứ, trợ giảng cho lớp"><span class="btn-add-pro-icon">+</span>Thêm GV</button>` : '';
  ...
  const caHocInfoHtml = gvChips.length
    ? gvChips.map(c=>
        `<span style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:20px;background:${c.bg};font-size:12.5px;white-space:nowrap">
  ...
  return `<div class="table-wrap">
    ...
    <div style="display:flex;border-top:2px solid #e4ebf5">
      <div style="width:34%;flex-shrink:0;display:flex;align-items:center;justify-content:center;gap:10px;padding:6px 16px;background:#f5f8fc;border-right:2px solid #e4ebf5;box-sizing:border-box;text-align:center">
        ...
        ${['admin','giaovien','trogiang'].includes(USER.role)?`<button class="btn-add-pro" onclick="openModalHV(null,'${escapeAttr(lop.tenLop)}')" title="Thêm học viên mới vào lớp"><span class="btn-add-pro-icon">+</span>Thêm HV</button>`:''}
      </div>
